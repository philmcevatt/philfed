...
# Add curated comps groups and select packages
# ... to end up with a stable KDE Gaming Desktop

############################################################
# VERSION
############################################################
PHILFED_VERSION="0.7.0"
...
# ensures script is run in sudo
# also carries forward user name and fedora version
############################################################
...
############################################################
# LOGGING
# Creates a full log and prints the review screen
# to .txt files on your desktop
############################################################
...
echo "Install asusctl/ROG Control Center: ${INSTALL_ASUSCTL}"
...
# Then an update before Nvidia & other installs.
############################################################
...
############################################################
# KDE FOUNDATION: Building the base of my system.
# Using Fedora's Comp Groups and their
...
# Individually curated packages from comp groups
# i did not install and the groups they came from
############################################################
...
CURATED_PACKAGES=(
  ...
  # Needed by any Gecko-based browser not just Firefox.
  libglvnd-gles
...
############################################################
# NVIDIA
# Installs RPM Fusion NVIDIA drivers.
# Builds akmods and regenerates initramfs.
# Warns about Secure Boot and protects against updated kernel
############################################################
...
############################################################
# VLC MEDIA PLAYER
# Installs VLC Media Player and its freeworld plugin set.
############################################################
section "VLC"
dnf -y install \
  vlc \
...
############################################################
# FLATPAK AND FLATHUB
# Discover cant install Flatpaks by default
# Flathub is registered here too.
############################################################
...
############################################################
# ASUS: ASUSCTL AND ROG CONTROL CENTER
# Repo setup, package installation and service enablement
############################################################
if [[ "${INSTALL_ASUSCTL}" == "true" ]]; then
  section "asusctl and ROG Control Center"
  ASUSCTL_READY=false
  if ! dnf -y install 'dnf5-command(copr)'; then
    warn "DNF COPR support could not be installed. Skipping asusctl."
  elif ! dnf -y copr enable lukenukem/asus-linux; then
    warn "asus-linux COPR could not be enabled. Skipping asusctl."
  elif ! dnf -y install asusctl rog-control-center; then
    warn "asusctl / rog-control-center could not be installed. Continuing without them."
  else
    ASUSCTL_READY=true
  fi

  if [[ "${ASUSCTL_READY}" == "true" ]]; then
    systemctl daemon-reload || note "daemon-reload reported an issue; continuing anyway."
    if systemctl enable --now asusd.service; then
      echo "asusd service enabled and started."
      complete_section "asusctl and ROG Control Center"
    else
      warn "asusctl installed, but asusd.service could not be enabled/started."
    fi
  fi
else
  skip_section "asusctl and ROG Control Center"
fi

############################################################
# BOOT TWEAKS
# Disables unnecessary boot waiting
############################################################
section "Boot tweaks"
