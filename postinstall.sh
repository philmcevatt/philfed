...
# ... andend up with a stable KDE Gaming Desktop
...
SUMMARY_FILE="${LOG_DIRECTORY}/postinstall-summary-${POSTINSTALL_VERSION}-${LOG_TIMESTAMP}.txt"
touch "${SUMMARY_FILE}"
...
############################################################
# CURATED PACKAGES
# curated packages base KDE does not install
# printing, small handy kde apps, torrents & passwords
############################################################
...
if dnf -y install "${CURATED_PACKAGES[@]}"; then
  ...
  warn "One or more Curated Packages could not be installed."
fi
...
############################################################
# MULTIMEDIA CODECS
# Installs Fedora's comp group codecs
# Plus aditional codecs not covered
# Swaps ffmpeg-free to ffmpeg
############################################################
...
############################################################
# FONTS
# Installs Fedora's Font comp group
# As well as Liberation fonts that LibreOffice uses.
############################################################
section "Fonts"
